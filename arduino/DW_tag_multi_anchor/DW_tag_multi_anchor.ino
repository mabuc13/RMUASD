...
#define DISTANCE_BETWEEN_ANCHORS 2.41
...
void updateStatistics(){
  // If we have enough measurements, ouput mean, standard deviation and stuff like that
  if(currentMeasurement >= MEASUREMENTS_PER_TURN){
    // Calculate mean, standard deviation and stuff

    float meanX,meanY,meanZ = 0.;
    for(int i = 0; i < MEASUREMENTS_PER_TURN; i++){
      meanX += xMeasurements[i];
      meanY += yMeasurements[i];
      meanZ += zMeasurements[i];
    }
    meanX /= (float) MEASUREMENTS_PER_TURN;
    meanY /= (float) MEASUREMENTS_PER_TURN;
    meanZ /= (float) MEASUREMENTS_PER_TURN;

    Serial.print("Mean x: "); Serial.println(meanX);
    Serial.print("Mean y: "); Serial.println(meanY);
    Serial.print("Mean z: "); Serial.println(meanZ);
    ...
    xMeasurements[currentMeasurement] = xyz_position[0];
    yMeasurements[currentMeasurement] = xyz_position[1];
    zMeasurements[currentMeasurement] = xyz_position[2];
